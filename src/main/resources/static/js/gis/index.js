require([
    ...
    uploadOpt.onclick = function () {
        ...
        if (files.length == 0) {
            ...
            if (file.size < 1024 * 100) { // 如果文件大小超過100KB就抱錯，沒有就繼續
                    ...
                    shp(reader.result).then(function (geojson) {
                        ...
                        layer.when(() => {
                            ...
                            document.getElementById('upload-content').innerHTML +=
                                ...
                                "<div class='col-4'><input type='range' class='custom-range uploadLayerOpacity' min='1' max='10' step='1' id='customRange3_" + uploadIdCnt + "'></div>" +
                                ...
                                "<button type='button' class='btn btn-outline-info btn-sm uploadLayerSearch' id='searchTo_" + uploadIdCnt + "'>查詢</button>" + // goto按鍵
                            ...
                            deleteUploadShp(map, UploadViewOpt);
			              	// ------------------------------------------------------------------ 20220623
			                let uploadSearchViewOpt = document.getElementsByClassName("uploadLayerSearch");
							// 新增查詢事件
			       			queryLayerByAttribute(map, view, 15, uploadSearchViewOpt, "uploadLyr");
							// ------------------------------------------------------------------
                            uploadIdCnt++;
                        });
                    });
                ...
                //console.log(reader.readAsText(file, 'big5'));
    ...
    layerChosenComboboxItem.addEventListener("calciteComboboxChange", function (event) {
		...
		let layerSearchViewOpt = document.getElementsByClassName("queryLayerBtn");
		queryLayerByAttribute(map, view, 20, layerSearchViewOpt, "lyr");
